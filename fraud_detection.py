...
for item in list(issuercountry_set):
    issuercountry_dict[item] = len(issuercountry_dict)
for item in list(txvariantcode_set):
    txvariantcode_dict[item] = len(txvariantcode_dict)
for item in list(currencycode_set):
    currencycode_dict[item] = len(currencycode_dict)
for item in list(shoppercountry_set):
    shoppercountry_dict[item] = len(shoppercountry_dict)
for item in list(interaction_set):
    interaction_dict[item] = len(interaction_dict)
for item in list(verification_set):
    verification_dict[item] = len(verification_dict)
for item in list(accountcode_set):
    accountcode_dict[item] = len(accountcode_dict)
...
import pandas as pd
#x_data=pd.DataFrame(data=x,  columns=['issuercountry', 'txvariantcode', 'issuer_id', 'amount', 'currencycode','shoppercountry', 'interaction', 'verification', 'cvcresponse', 'creationdate_stamp','accountcode', 'mail_id', 'ip_id', 'card_id'])
#
...
enc = OneHotEncoder(categorical_features=[0,1,4,5,6,7,10],
       handle_unknown='error', n_values='auto', sparse=True)
enc.fit(x)  
x_transformed=enc.transform(x).toarray()
from imblearn.over_sampling import SMOTE 

#y_score_no_sampling = classifier.predict_proba(enc.transform(x_test)[:, 1]


x_train, x_test, y_train, y_test_no_sampling = train_test_split(x_transformed, y, test_size = 0.2)#test_size: proportion of train/test data
from sklearn import metrics
y_score_no_sampling = classifier.fit(x_train, y_train).decision_function(x_test)

sm = SMOTE(ratio=0.1,random_state=42)
x_res, y_res = sm.fit_sample(x_transformed, y)
x_train, x_test, y_train, y_test10 = train_test_split(x_res, y_res, test_size = 0.2)#test_size: proportion of train/test data
y_score_10 = classifier.fit(x_train, y_train).decision_function(x_test)

sm = SMOTE(ratio=0.2,random_state=42)
x_res, y_res = sm.fit_sample(x_transformed, y)
x_train, x_test, y_train, y_test20 = train_test_split(x_res, y_res, test_size = 0.2)#test_size: proportion of train/test data
y_score_20 = classifier.fit(x_train, y_train).decision_function(x_test)

sm = SMOTE(ratio=0.3,random_state=42)
x_res, y_res = sm.fit_sample(x_transformed, y)
x_train, x_test, y_train, y_test30 = train_test_split(x_res, y_res, test_size = 0.2)#test_size: proportion of train/test data
y_score_30 = classifier.fit(x_train, y_train).decision_function(x_test)

sm = SMOTE(ratio=0.4,random_state=42)
x_res, y_res = sm.fit_sample(x_transformed, y)
x_train, x_test, y_train, y_test40 = train_test_split(x_res, y_res, test_size = 0.2)#test_size: proportion of train/test data
y_score_40 = classifier.fit(x_train, y_train).decision_function(x_test)

sm = SMOTE(ratio=0.5,random_state=42)
x_res, y_res = sm.fit_sample(x_transformed, y)
x_train, x_test, y_train, y_test50 = train_test_split(x_res, y_res, test_size = 0.2)#test_size: proportion of train/test data
...
fpr[0], tpr[0], _ = metrics.roc_curve(y_test_no_sampling[:], y_score_no_sampling[:])
roc_auc[0] = metrics.auc(fpr[0], tpr[0])
fpr[0], tpr[0], _ = metrics.roc_curve(y_test_no_sampling[:], y_score_no_sampling[:])
roc_auc[0] = metrics.auc(fpr[0], tpr[0])

fpr[1], tpr[1], _ = metrics.roc_curve(y_test10[:], y_score_10[:])
roc_auc[1] = metrics.auc(fpr[1], tpr[1])
fpr[1], tpr[1], _ = metrics.roc_curve(y_test10[:], y_score_10[:])
roc_auc[1] = metrics.auc(fpr[1], tpr[1])

fpr[2], tpr[2], _ = metrics.roc_curve(y_test20[:], y_score_20[:])
roc_auc[2] = metrics.auc(fpr[2], tpr[2])
fpr[2], tpr[2], _ = metrics.roc_curve(y_test20[:], y_score_20[:])
roc_auc[2] = metrics.auc(fpr[2], tpr[2])

fpr[3], tpr[3], _ = metrics.roc_curve(y_test30[:], y_score_30[:])
roc_auc[3] = metrics.auc(fpr[3], tpr[3])
fpr[3], tpr[3], _ = metrics.roc_curve(y_test30[:], y_score_30[:])
roc_auc[3] = metrics.auc(fpr[3], tpr[3])

fpr[4], tpr[4], _ = metrics.roc_curve(y_test40[:], y_score_40[:])
roc_auc[4] = metrics.auc(fpr[4], tpr[4])
fpr[4], tpr[4], _ = metrics.roc_curve(y_test40[:], y_score_40[:])
roc_auc[4] = metrics.auc(fpr[4], tpr[4])

fpr[5], tpr[5], _ = metrics.roc_curve(y_test50[:], y_score_50[:])
...
for item in data:
    if(item[14]==1):
        data2.append(item)
transaction_num_day2=np.zeros(len(transaction_num_day))
for item in data2:
    for i in range(len(day_aggregate)):
        if day_aggregate[i][0][15]==item[15]:
            index=i
            break
    transaction_num_day2[index]=transaction_num_day2[index]+1
    
plt.figure(1)
transaction_percentage=np.zeros(len(transaction_num_day))

for i in range(len(transaction_percentage)):
    transaction_percentage[i]=transaction_num_day2[i]/transaction_num_day[i]
    
plt.plot(transaction_num_day2, color = 'c', linewidth = 2)
plt.plot()
plt.text(2,0.0,'Date: 2015-10-8')
plt.xlabel('Date')
plt.ylabel('Number of Transactions')
